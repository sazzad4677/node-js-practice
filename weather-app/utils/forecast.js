...
const forecast = (longitude, latitude, cb) => {
    ...
    request({ url: url, json: true },(error, response) => {
        if (error) {
            ...
            cb('', `${response.body.current.weather_descriptions[0]}. it is currently ${response.body.current.temperature} degrees out. It feels like ${response.body.current.feelslike} degrees out`)
        }
    })
}
